...
var tpsCounter = 0
var tpsLastSecond = Date.now()
var currentTPS = 0
var currentMSTP = 0      // 单次 tick 执行耗时（ms）
// Don't change this
const TMT_VERSION = {
	tmtNum: "2.6.6.2",
...
var interval = setInterval(function() {
	...
	currentMSTP = Date.now() - tickStart
	tpsCounter++
	let tpsNow = Date.now()
	if (tpsNow - tpsLastSecond >= 1000) {
		currentTPS = tpsCounter / ((tpsNow - tpsLastSecond) / 1000)
		tpsCounter = 0
		tpsLastSecond = tpsNow
	}
	let tpsEl = document.getElementById('tps-display')
	if (tpsEl) tpsEl.textContent = `TPS: ${currentTPS.toFixed(1)} | MSTP: ${currentMSTP}ms`
