let modInfo = {
	name: "反物质维度：新视野",
	nameI18N: "反物质维度：新视野",// When you enabled the internationalizationMod, this is the name in the second language
	id: "ADNH",
	author: "noUshio_Noa_official",
	pointsName: "points",
...
function addedPlayerData() { return {
    settings: { coolDownsEnabled: true },
    cooldowns: {},
}}

function toggleCdOpt() {
    if (!player.settings) Vue.set(player, "settings", {})
    let cur = player.settings.coolDownsEnabled !== false
    Vue.set(player.settings, "coolDownsEnabled", !cur)
...
function fixOldSave(oldVersion){
    if (!player.settings) Vue.set(player, "settings", {})
    if (player.settings.coolDownsEnabled === undefined)
        Vue.set(player.settings, "coolDownsEnabled", true)
    if (!player.cooldowns) Vue.set(player, "cooldowns", {})
}